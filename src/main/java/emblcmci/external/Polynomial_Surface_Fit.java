...
public class Polynomial_Surface_Fit implements PlugInFilter {
    ...
    public void run(ImageProcessor ip) {
        ...
        Ny = roi.height;
        Nx = roi.width;
        TheWidth = imp.getWidth();
        TheHeight = imp.getHeight();
        ...
        cSlice = imp.getCurrentSlice(); // remember current slice
        ...
        imp.setSlice(cSlice);	// reset the the original slice
        // If the image is a Stack than create an average frame
        if( Nt>1 ) {
            for(iy=0; iy<Ny; iy++) {
                for(ix=0; ix<Nx; ix++) {
                    TheImage[iy][ix] /= Nt;
        ...
        ImagePlus TheFit_imp = NewImage.createFloatImage(imp.getTitle()+AStr, Nx, Ny,1, NewImage.FILL_BLACK);
